...
from gcn_model import GCNEvolutionModel
...
def train_val_test(data_list, hidden_channels, in_features, out_features, num_epochs, learning_rate):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    avg_test_loss = 0
    avg_relative_mse = 0
    avg_correlation = 0
    ...
    for data in data_list:
        model = GCNEvolutionModel(in_features=in_features, hidden_channels=hidden_channels, out_features=out_features).to(device)
        ...
        p_value, correlation = kendall_tau(data.y[data.test_mask], out[data.test_mask].detach().numpy())
